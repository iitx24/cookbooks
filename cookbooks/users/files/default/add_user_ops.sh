...
HOSTNAME=`hostname`
USERS=`grep asgops /etc/sudoers | wc -l`
UNAME=`uname`
...
if [ "$USERS" == "0" ]; then
        ...
        echo "%asgops ALL=(ALL) NOPASSWD: ALL" >> /etc/sudoers
...
grep ASGOPS /etc/passwd > /var/tmp/pw1
...
if [ "Linux" = "$UNAME" ]; then
	for i in `cat $FILE1`
        	do
        		for pw1 in `grep $i $FILE1|cut -c1-2`;do  echo $pw1"test3456" | passwd --stdin "$i";done
